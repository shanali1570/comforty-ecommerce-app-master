...
const WishlistLink: React.FC = () => {
  ...
  return (
  <div className=' w-[140px] h-12 font-bold hoverEffect hover:bg-darkOrange hover:text-white bg-white rounded-md flex items-center justify-center gap-2'>
    <Link href={"/wishlist"} className=' flex items-center justify-center gap-2'>
      <CiHeart className='text-[20px] '/>
      <span className='text-14 '>Wishlist</span>
    
    <div className='bg-[#007580] rounded-full w-[20px] h-[20px] text-white flex items-center justify-center'>
        <p className='text-[14px] text-center'>{wishlist ? wishlist?.length : 0}</p>
    </div>
    </Link>
